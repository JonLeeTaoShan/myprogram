...
**        body of text.
**     3) Suport is inclpuded to handle upper and lower case matching.     
...
//extern int nline=1;
// int nline=1 ÒÑ¾­ÔÚ±ðµÄÎÄ¼þÖÐ¶¨Òå¹ýÁË£¬ÔÚÕâÀïÖ»ÊÇÒýÓÃ
/*
* Malloc the AC Memory
*/ 
static void *AC_MALLOC (int n)     //void±íÊ¾Î´È·¶¨ÀàÐÍµÄÖ¸Õë
{
	void *p;          //void *malloc(unsigned int n); fenpei changdu wei n zi jie de nei chun kuai
...
/*
*    Simple QUEUE NODE     ¼òµ¥µÄ¶ÓÁÐ½Úµã
*/ 
...
/*
*    Simple QUEUE Structure  ¼òµ¥µÄ¶ÓÁÐ½á¹¹
*/ 
...
/*
*Init the Queue   ³õÊ¼»¯¶ÓÁÐ
*/ 
static void queue_init (QUEUE * s) 
{
	s->head = s->tail = 0;  // Ö¸Ïò×î¿ªÊ¼µÄ0Î»ÖÃ
	s->count = 0;
}
...
static void queue_add (QUEUE * s, int state) 
{
	...
	if (!s->head)
	{
		...
		MEMASSERT (q, "queue_add");  // it has defined before
		q->state = state;
...
static void AddPatternStates (ACSM_STRUCT * acsm, ACSM_PATTERN * p) 
{
	...
	*/ 
	for (; n > 0; pattern++, n--)
	{
		...
		if (next == ACSM_FAIL_STATE)  // ACSM_FAIL_STATE=-1
			break;
		state = next;
	}
...
static unsigned char Tc[1024];
/*
*   Search Text or Binary Data for Pattern matches
*/ 
int acsmSearch (ACSM_STRUCT * acsm, unsigned char *Tx, int n,void (*Match) (ACSM_PATTERN * mlist, void *data,int *id),
				void *data,int *id)
{
	int state;
	ACSM_PATTERN * mlist;
...
void PrintMatch (ACSM_PATTERN * pattern,ACSM_PATTERN * mlist, int nline,int index) 
{
	...
	if(mlist->nocase)
		fprintf (stdout, "Match KeyWord %s id %d at %d line %d char\n", mlist->patrn,mlist->id,nline,index);
	else
		fprintf (stdout, "Match KeyWord %s id %d at %d line %d char\n", mlist->casepatrn,mlist->id,nline,index);
...
void Match(ACSM_PATTERN * mlist, void *data, int *id)
{
	...
	if(mlist)
	{
		sprintf(pparse->proto_name,"%s",mlist->udata);
		*id = mlist->id;
	}
}